        ...
        private Task ConsumeTopicPartitionAsync(string topic, int partitionId)
        {
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    _options.Log.DebugFormat("Consumer: Creating polling task for topic: {0} on partition: {1}", topic, partitionId);
                ...
                {
                    _options.Log.DebugFormat("Consumer: Disabling polling task for topic: {0} on partition: {1}", topic, partitionId);
                    Task tempTask;

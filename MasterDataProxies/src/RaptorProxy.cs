...
namespace MasterDataProxies
{
  ...
  public class RaptorProxy : BaseProxy, IRaptorProxy
  {
    ...
    /// <returns></returns>
    public async Task<ContractExecutionResult> AddFile(long? projectId, Guid? projectUid, string fileDescriptor, long fileId, IDictionary<string, string> customHeaders = null)
    {
      var urlKey = "RAPTORADDFILE_API_URL";
      string url = configurationStore.GetValueString(urlKey);
      log.LogDebug($"RaptorProxy.AddFile: urlKey: {urlKey}  url: {url} customHeaders {customHeaders}");

      log.LogDebug($"RaptorProxy.AddFile: projectId: {projectId} projectUid: {projectUid} fileDescriptor {fileDescriptor} fileId {fileId}");
      var queryParams = $"?projectId={projectId}&projectUid={projectUid}&fileDescriptor={fileDescriptor}&fileId={fileId}";
    ...
    /// <param name="fileId">A unique file identifier</param>
    /// <returns></returns>
    public async Task<ContractExecutionResult> DeleteFile(long? projectId, Guid? projectUid, string fileDescriptor, long fileId, IDictionary<string, string> customHeaders = null)
    {
      var urlKey = "RAPTORDELETEFILE_API_URL";
      string url = configurationStore.GetValueString(urlKey);
      log.LogDebug($"RaptorProxy.DeleteFile: urlKey: {urlKey}  url: {url} customHeaders {customHeaders}");

      log.LogDebug($"RaptorProxy.DeleteFile: projectId: {projectId} projectUid: {projectUid} fileDescriptor {fileDescriptor} fileId {fileId}");
      var queryParams = $"?projectId={projectId}&projectUid={projectUid}&fileDescriptor={fileDescriptor}&fileId={fileId}";

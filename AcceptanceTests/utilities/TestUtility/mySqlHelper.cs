...
namespace TestUtility
{
    public class MySqlHelper
    {
        ...
        /// <summary>
        /// Delete all records from the database for an assetid
        /// </summary>
        /// <param name="assetUid"></param>
        public void DeleteAllRecordsForAnAsset(string assetUid)
        {
            var mysqlHelper = new MySqlHelper();
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.Asset WHERE AssetUID = '{assetUid}';"); 
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.AssetBookmarks WHERE AssetUID = '{assetUid}';");     
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.AssetConfiguration WHERE AssetUID = '{assetUid}';");    
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.AssetUTCOffset WHERE AssetUID = '{assetUid}';"); 
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.OdometerMeterEvent WHERE AssetUID = '{assetUid}';");     
            mysqlHelper.ExecuteMySqlInsert(appConfig.dbConnectionString, $@"DELETE FROM `{appConfig.dbSchema}`.TimeStampedEvent WHERE AssetUID = '{assetUid}';");                                      
        }

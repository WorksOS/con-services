...
namespace VSS.TRex.Tests.Analytics.MDPStatistics.GridFabric
{
  public class MDPResponseTests : BaseTests
  {
    ...
    public void Test_MDPResponse_ConstructResult_Successful()
    {
      ...
      Assert.True(Math.Abs(result.ConstantTargetMDP - _response.LastTargetMDP) < Consts.TOLERANCE, "Invalid initial result value for ConstantTargetMDP.");
      Assert.True(Math.Abs(result.AboveTargetPercent - _response.ValueOverTargetPercent) < Consts.TOLERANCE, "Invalid initial result value for AboveMDPPercent.");
      Assert.True(Math.Abs(result.WithinTargetPercent - _response.ValueAtTargetPercent) < Consts.TOLERANCE, "Invalid initial result value for WithinMDPPercent.");
      Assert.True(Math.Abs(result.BelowTargetPercent - _response.ValueUnderTargetPercent) < Consts.TOLERANCE, "Invalid initial result value for BelowMDPPercent.");

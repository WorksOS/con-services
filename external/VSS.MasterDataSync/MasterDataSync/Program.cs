...
namespace VSS.Nighthawk.MasterDataSync
{
	public class Program
	{
		...
		private static readonly string serviceName = ConfigurationManager.AppSettings.Get("ServiceName");

		private static void Main(string[] args)
		{
			var exitCode = HostFactory.Run(host =>
			{
				host.SetServiceName(string.IsNullOrEmpty(serviceName) ? "_NHMasterDataSyncService" : serviceName);
				host.SetDisplayName(string.IsNullOrEmpty(serviceName) ? "_NHMasterDataSyncService" : serviceName);

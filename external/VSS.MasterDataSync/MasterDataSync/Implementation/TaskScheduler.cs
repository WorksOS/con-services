...
namespace VSS.Nighthawk.MasterDataSync.Implementation
{
    public class TaskScheduler
    {
        ...
        public TaskScheduler(IHttpRequestWrapper httpRequestWrapper, IConfigurationManager configurationManager, ICacheManager cacheManager, ITpassAuthorizationManager tpassAuthorizationManager)
        {
            ...
            tasks = _configurationManager.GetAppSetting("Tasks").Split(',').ToList();
        }
            ...
            try
            {
                if (!_isServiceStopped)
                {
                    using (var opCtx = ObjectContextFactory.NewNHContext<INH_OP>())
                    {
                        ...
                        var taskToProcess =
                          opCtx.MasterDataSyncReadOnly.Where(i => (tasks.Contains(i.TaskName)) && (!i.InProgress || (i.InProgress && DbFunctions.DiffMinutes(i.StartUTC, DateTime.UtcNow) >= TaskTimeOutInterval)) && i.IsActive)
                            .OrderBy(u => u.EndUTC)
                            .FirstOrDefault();
                        ...
                        if (taskToProcess != null)
                        {
                            ...
                            switch (taskName)
                            {
                                case StringConstants.APIUserTask:
                                    var apiUserSyncProcessor = new APIUserSyncProcessor(taskName, _httpRequestWrapper, _configurationManager, _cacheManager, _tpassAuthorizationManager);
                                    _isDataProcessed = apiUserSyncProcessor.Process(ref _isServiceStopped);
                                    break;

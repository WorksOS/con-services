...
namespace VSS.Hosted.VLCommon
{
    public partial class PMInterval
    {
        ...
        public virtual ICollection<PMCheckListStep> PMCheckListStep
        {
            get
            {
                if (_pMCheckListStep == null)
                {
                    var newCollection = new FixupCollection<PMCheckListStep>();
                    newCollection.CollectionChanged += FixupPMCheckListStep;
                    _pMCheckListStep = newCollection;
                }
                return _pMCheckListStep;
            }
            set
            {
                if (!ReferenceEquals(_pMCheckListStep, value))
                {
                    var previousValue = _pMCheckListStep as FixupCollection<PMCheckListStep>;
                    if (previousValue != null)
                    {
                        previousValue.CollectionChanged -= FixupPMCheckListStep;
                    }
                    _pMCheckListStep = value;
                    var newValue = value as FixupCollection<PMCheckListStep>;
                    if (newValue != null)
                    {
                        newValue.CollectionChanged += FixupPMCheckListStep;
                    }
                }
            }
        }
        private ICollection<PMCheckListStep> _pMCheckListStep;
        ...
        private void FixupPMCheckListStep(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (PMCheckListStep item in e.NewItems)
                {
                    item.PMInterval = this;
                }
            }
            ...
            if (e.OldItems != null)
            {
                foreach (PMCheckListStep item in e.OldItems)
                {
                    if (ReferenceEquals(item.PMInterval, this))
                    {
                        item.PMInterval = null;

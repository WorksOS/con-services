...
using VSS.MasterData.WebAPI.PreferenceRepository;
...
namespace VSS.MasterData.WebAPI.Preference
{
	...
	public class Startup
	{
		...
		public Startup(IHostEnvironment env)
		{
			...
			var logger = new LoggerConfiguration().ReadFrom.Configuration(Configuration).MinimumLevel.Information()
		...
		public void ConfigureServices(IServiceCollection services)
		{
			...
			{
				options.AddPolicy("VSS", corsBuilder => corsBuilder.AllowAnyOrigin().AllowAnyHeader().AllowAnyMethod());
			});
		...
		public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
		{
			...
			app.UseSwaggerUI(c =>
			{
				c.SwaggerEndpoint("/swagger/v1/swagger.json", "VSS.MasterData.WebAPI V1");
			});
				...
				using (var reader = new StreamReader(context.Request.Body, Encoding.UTF8, false, 1024, true))
				{
						...
						!context.Request.Path.ToString().ToLower().Contains("swagger"))
					{
						...
						Logger.LogInformation(
							$"vlmd_preference_Request : {context.Request.Method} {context.Request.Scheme}://{context.Request.Host}{context.Request.Path}{context.Request.QueryString} {body}");

...
namespace VSS.Hosted.VLCommon.Services.MDM
{
    public class GeofenceService : ServiceBase, IGeofenceService
    {
        ...
        private static readonly string PurgePublishApiBaseUri = ConfigurationManager.AppSettings["PurgePublish.WebAPIURI"];
    ...
    public GeofenceService()
        ...
        public bool PurgePublish(object geofencePurge)
        {
          try
          {
            var stringified = JsonConvert.SerializeObject(geofencePurge, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
            var success = DispatchRequest(PurgePublishApiBaseUri, HttpMethod.Post, stringified);
            _log.IfDebugFormat("Geofence Purge Event Payload Data : {0} Posted Status : {1}", stringified, success ? "Success" : "Failed");
            return success;
          }
          catch (Exception ex)
          {
            _log.IfWarnFormat("Error occurred while purging Geofence in VSP stack. Error message :{0}",
                            ex.Message);
            return false;
          }
        }

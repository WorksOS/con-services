...
{
  public class CancelledServicePlanConfiguration : Activity
  {
    ...
    public override ActivityResult Execute(Inputs inputs)
    {
      ...
      try
      {
        success = Services.ServiceViews().ConfigureForCancelledServicePlans(
          context.ExistingDeviceAsset.AssetId,
          context.ActionUTC,
          context.ExistingDeviceAsset.GpsDeviceId,
          context.ExistingDeviceAsset.Type.Value,
          context.ServiceType.Value, context.ExistingService.ServiceID
          );

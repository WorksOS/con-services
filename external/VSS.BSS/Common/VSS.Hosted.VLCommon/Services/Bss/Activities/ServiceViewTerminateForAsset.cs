...
{
  public class ServiceViewTerminateForAsset : Activity
  {
    ...
    public override ActivityResult Execute(Inputs inputs)
    {
      ...
      try
      {
        terminatedViews = Services.ServiceViews().TerminateServiceViewsForAsset(context.Asset.AssetId, context.TransferDate);
      }
      catch (Exception ex)
      {

...
{
  public class PLFault : NestedMessage
  {
    public byte Level;
    public byte MID;
    public ushort EID;
    ...
    public override void Serialize(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      ...
      BigEndianSerializer(action, raw, ref bitPosition, 2, ref EID);
  ...
  public class FaultEvent : PLTrackerMessage
  {
    public static new readonly int kPacketID = 0x34;
    ...
    public override void Serialize(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      ...
      int count = Faults == null ? 0 : Faults.Length;
      serializer(action, raw, ref bitPosition, 8, ref count);
      if (action == SerializationAction.Hydrate)
        Faults = new PLFault[count];

      Faults = (PLFault[])
            serializeHomogeneousRunLengthArray(action, raw, ref bitPosition, 0, Faults, typeof(PLFault));
    }
  }
}

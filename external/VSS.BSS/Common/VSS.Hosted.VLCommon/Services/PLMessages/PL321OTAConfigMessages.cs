...
{
  public class PL321OTAConfigMessages : PLBaseMessage
  {
    ...
    public override void Serialize(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      ...
      {
        HydrateOTAReportingMessage(action, raw, ref bitPosition);
      }
      else
      {
        serializeOTAMessage(action, raw, ref bitPosition);
      }
    }
    ...
    #region Serialize OTA Message Methods

    private void serializeOTAMessage(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      uint numParamsPos = bitPosition;
      bitPosition += 8;
      byte numParams = 0;
      SerializeSMUAdj(action, raw, ref bitPosition, ref numParams);
      SerializeReportingIntervals(action, raw, ref bitPosition, ref numParams);
      SerializeDigitalInputs(action, raw, ref bitPosition, ref numParams);
      if (action == SerializationAction.Serialize)
        serializer(action, raw, ref numParamsPos, 8, ref numParams);
    }

    private void SerializeDigitalInputs(SerializationAction action, byte[] raw, ref uint bitPosition, ref byte numParams)
    {
    ...
    private void SerializeSMUAdj(SerializationAction action, byte[] raw, ref uint bitPosition, ref byte paramCount)
    {
    ...
    private void SerializeReportingIntervals(SerializationAction action, byte[] raw, ref uint bitPosition, ref byte numParams)
    {
      ...
      if (diagnosticTransmissionFrequencyFieldID == (byte)FieldID.DiagnosticTransmissionFrequency)
      {
        ...
        serializeFixedLengthString(action, raw, ref bitPosition, 1, ref diagnosticTransmissionFrequencyStr, true);
        numParams++;
      }
      ...
      if (positionReportConfigFieldID == (byte)FieldID.PositionReportConfiguration)
      {
    ...
    #region Hydrate OTA Message Methods

    private void HydrateOTAReportingMessage(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      uint numParams = 0;
      serializer(action, raw, ref bitPosition, 8, ref numParams);
      for (int i = 0; i < numParams; i++)
      {
        HydrateReportingField(action, raw, ref bitPosition);
      }
    }

    private void HydrateReportingField(SerializationAction action, byte[] raw, ref uint bitPosition)
    {
      byte fieldID = 0;
      ...
      switch ((FieldID)fieldID)
      {
        ...
        case FieldID.DiagnosticTransmissionFrequency:
          ...
          serializeFixedLengthString(action, raw, ref bitPosition, 1, ref diag, isSRDiagnostic: true);
        ...
        case FieldID.SMUFuelReporting:
          ...
          serializer(action, raw, ref bitPosition, 8, ref smuFuelReporting);
          break;
        case FieldID.PositionReportConfiguration:
          positionReportConfigFieldID = (byte)FieldID.PositionReportConfiguration;
          serializer(action, raw, ref bitPosition, 8, ref positionReportConfig);
          break;

...
using VSS.Nighthawk.MTSGateway.Common.Interfaces;

namespace VSS.Nighthawk.MTSGateway.Common.Helpers
{
  ...
  public class RabbitMqConnectionConfig : IConnectionConfig
  {
    ...
    public string ConnectionString(string csName = "NH_RABBITMQ", string keyVirtualHost = "RabbitMqVirtualHost", string keyQName = "RabbitMqName")
    {
      ...
      string dataOutRabbitMqVirtualHost = ConfigurationManager.AppSettings[machineRabbitMqVirtualHost] ?? ConfigurationManager.AppSettings[keyVirtualHost];
      string dataOutRabbitMqName = ConfigurationManager.AppSettings[machineRabbitMqName] ?? ConfigurationManager.AppSettings[keyQName];

      connString = connString.Replace(ConfigConstants.MqHostPh, dataOutRabbitMqVirtualHost);
      connString = connString.Replace(ConfigConstants.MqNamePh, dataOutRabbitMqName);
    ...
    public ushort GetHeartbeatSeconds()
    {
      ...
      // This is done like this mainly so a developer can force the db connection string to contain their own creds, for debugging purposes.
      string heartbeatString = ConfigurationManager.AppSettings[machineRabbitMqHeartbeatSeconds] ?? ConfigurationManager.AppSettings["RabbitMqHeartbeatSeconds"];

      ushort heartbeat;
      if (string.IsNullOrWhiteSpace(heartbeatString) || !ushort.TryParse(heartbeatString, out heartbeat))
      {

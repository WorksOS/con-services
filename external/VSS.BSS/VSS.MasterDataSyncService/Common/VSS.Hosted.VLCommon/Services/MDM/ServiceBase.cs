    ...
    protected bool DispatchRequest(string endpointUri, HttpMethod methodType, string payload = null, List<KeyValuePair<string, string>> headers = null)
    {
      ...
      switch (httpResponseMessage.StatusCode)
      {
        ...
        case HttpStatusCode.Conflict:
          var responseMsg = httpResponseMessage.Content.ReadAsStringAsync().Result;
          _log.IfInfoFormat("Status Code : {0} Response {1} ", HttpStatusCode.Conflict, responseMsg);
          if (payload != null && payload.Contains("CreateAssetEvent"))
          {
            Guid assetGuid;
            if (!Guid.TryParse(responseMsg, out assetGuid))
              return true;
            var createAssetEvent = JsonHelper.DeserializeJsonToObject<CreateAssetEvent>(payload);
            using (var opCtx = ObjectContextFactory.NewNHContext<INH_OP>())
            {
              var asset = opCtx.Asset.First(a => a.AssetID == createAssetEvent.LegacyAssetID);
              asset.AssetUID = assetGuid;
              string mk_Sno = string.Concat(createAssetEvent.MakeCode, "_", createAssetEvent.SerialNumber);
              var assetRef = opCtx.AssetReference.Where(x => x.Value == mk_Sno).ToList();
              if (assetRef.Any())
              {
                _log.InfoFormat("updating asset Reference with Value {0} with AssetUID {1}", mk_Sno, responseMsg);

node ('jenkinsslave-pod') {
    ...
    try {
        stage('Test Solution') {
                ...
                sh 'docker run -it -v ${env.WORKSPACE}/TestResults:/TestResults ${building.id} bash -c "ls /build/tests/*/*/*netcore*.csproj | xargs -I@ -t dotnet test  --test-adapter-path:. --logger:"xunit;LogFilePath=/TestResults/@.xml" @'

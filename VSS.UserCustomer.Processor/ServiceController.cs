...
namespace VSS.UserCustomer.Processor
{
  ...
  {
    private readonly IUserCustomerProcessor _consumer;
    private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);

    public ServiceController(IUserCustomerProcessor consumer)
    {
      _consumer = consumer;
    }
    ...
    public void Start()
    {
      ...
      {
        Log.Info(string.Format("Failed to start Customer Processor.. \n {0} \n {1}", ex.Message, ex.StackTrace));
      }
      Log.Info("Customer Processor has been Started");

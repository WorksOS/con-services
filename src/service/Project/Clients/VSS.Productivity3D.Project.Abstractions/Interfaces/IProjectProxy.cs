...
namespace VSS.Productivity3D.Project.Abstractions.Interfaces
{
  public interface IProjectProxy : ICacheProxy
  {
    ...
    Task<List<ProjectData>> GetProjects(string accountUid, IDictionary<string, string> customHeaders = null);

    Task<List<ProjectData>> GetIntersectingProjects(string accountUid, 
    ...
    Task<ProjectData> GetProjectForAccount(string accountUid, string projectUid,
      IDictionary<string, string> customHeaders = null);

    //To support 3dpm v1 end points which use legacy project id
    Task<ProjectData> GetProjectForAccount(string accountUid, long shortRaptorProjectId,
      IDictionary<string, string> customHeaders = null);

...
namespace IntegrationTests.WebApiTests.FileImportTests
{
  public class FileImportTests_Alignment : WebApiTestsBase
  {
    ...
    public async Task TestImportANewFileThenUpdateTheAlignmentFile(string uriRoot)
    {
      ...
      await importFile.SendRequestToFileImportV4(ts, importFileArray, 2, new ImportOptions(HttpMethod.Post, new[] { $"filename={importFilename}" }), HttpStatusCode.BadRequest);
      var expectedResult2 = importFile.ExpectedImportFileDescriptorSingleResult.ImportedFileDescriptor;
      var importFileList = await importFile.GetImportedFilesFromWebApi<ImportedFileDescriptorListResult>($"api/v4/importedfiles?projectUid={projectUid}", customerUid);
    ...
    public async Task TestImportANewFileThenDeleteTheAlignmentFile(string uriRoot)
    {
      ...
      await importFile.SendRequestToFileImportV4(ts, importFileArray, 1, new ImportOptions(HttpMethod.Delete));
      var importFileList = await importFile.GetImportedFilesFromWebApi<ImportedFileDescriptorListResult>($"api/v4/importedfiles?projectUid={projectUid}", customerUid);
    ...
    public async Task TestImportTheSameFileTwice(string uriRoot)
    {
      ...
      var filesResult2 = await importFile.SendRequestToFileImportV4(ts, importFileArray, 2, new ImportOptions(HttpMethod.Post, new[] { $"filename={importFilename}" }), HttpStatusCode.BadRequest);

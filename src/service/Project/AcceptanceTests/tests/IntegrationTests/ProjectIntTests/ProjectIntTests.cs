...
namespace IntegrationTests.ProjectIntTests
{
  public class ProjectIntTests
  {
    ...
    [Fact]
    public async Task Create_Project_Then_Retrieve()
    {
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, 1);
     ...
     $"| false      | {projectName} | New Zealand Standard Time | {ProjectType.Standard} | {startDate:O}    | {endDate:O}    | {projectGuid}  | {projectId}  | {customerGuid}   | {projectId}      | {_polygon1}     |" };
      ...
      await ts.GetProjectsViaWebApiV3AndCompareActualWithExpected(HttpStatusCode.OK, customerGuid, expectedProjects);
    }

    [Fact]
    public async Task Create_Multiple_Projects_And_Inject_Required_Data_Then_Retrieve()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid1, projectId1, projectName, startDate, endDate, 1);
      await Create_And_Subscribe_AdditionalProjects_for_existing_Customer(ts, customerGuid, projectGuid2, projectId2, projectName, startDate, endDate, 2);
    ...
    [Fact]
    public async Task Create_Then_Update_Project_Name()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, 1);
      await ts.UpdateProjectViaWebApiV3(projectGuid, "New Name", endDate, "New Zealand Standard Time", DateTime.UtcNow, HttpStatusCode.OK);
    ...
    [Fact]
    public async Task Create_Then_Update_Project_EndDate()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, 1, "Central Standard Time");

      await ts.UpdateProjectViaWebApiV3(projectGuid, projectName, endDate.AddDays(10), "Central Standard Time", DateTime.UtcNow, HttpStatusCode.OK);
      var expectedProjects = new[] {
      "| IsArchived | ProjectName   | ProjectTimezone           | ProjectType            | ProjectStartDate | ProjectEndDate          | ProjectUID    | ProjectID     | CustomerUID      | LegacyCustomerId | ProjectBoundary | ",
    ...
    [Fact]
    public async Task Create_Then_Try_Update_Project_TimeZone()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await ts.UpdateProjectViaWebApiV3(projectGuid, projectName, endDate, "Central Standard Time", DateTime.UtcNow, HttpStatusCode.Forbidden);
      var expectedProjects = new[] {
      "| IsArchived | ProjectName   | ProjectTimezone           | ProjectType            | ProjectStartDate | ProjectEndDate | ProjectUID    | ProjectID     | CustomerUID      | LegacyCustomerId | ProjectBoundary | ",
     $"| false      | {projectName} | New Zealand Standard Time | {ProjectType.Standard} | {startDate:O}    | {endDate:O}    | {projectGuid} | {projectId}   | {customerGuid}   | {projectId}      | {_polygon1} |" };
      await ts.GetProjectsViaWebApiV3AndCompareActualWithExpected(HttpStatusCode.OK, customerGuid, expectedProjects);
    ...
    [Fact]
    public async Task Create_Project_Then_Update_Project_Type()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await ts.UpdateProjectViaWebApiV3(projectGuid, projectName, endDate, "New Zealand Standard Time", DateTime.UtcNow, HttpStatusCode.OK, ProjectType.ProjectMonitoring);
      var expectedProjects = new[] {
      "| IsArchived | ProjectName   | ProjectTimezone            | ProjectType                     | ProjectStartDate | ProjectEndDate | ProjectUID    | ProjectID   | CustomerUID      | LegacyCustomerId | ProjectBoundary | ",
    ...
    [Fact]
    public async Task Create_Then_Delete_Project()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
     ...
     $"| false      | {projectName} | New Zealand Standard Time | {ProjectType.Standard} | {startDate:O}    | {endDate:O}    | {projectGuid} | {projectId} | {customerGuid}   | {projectId}      | {_polygon1} |" };
      await ts.GetProjectsViaWebApiV3AndCompareActualWithExpected(HttpStatusCode.OK, customerGuid, expectedProjects);
      await ts.DeleteProjectViaWebApiV3(projectGuid, HttpStatusCode.OK);
    ...
    [Fact]
    public async Task Create_Then_Associate_Geofence_with_Project()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await ts.AssociateGeofenceProjectViaWebApiV3(projectGuid, geofenceGuid, DateTime.UtcNow, HttpStatusCode.OK);
    ...
    [Fact]
    public async Task Create_Then_Associate_Multiple_NonProject_Type_Geofences_with_Project()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await ts.AssociateGeofenceProjectViaWebApiV3(projectGuid, geofenceGuid2, DateTime.UtcNow, HttpStatusCode.OK);
      ...
      await ts.AssociateGeofenceProjectViaWebApiV3(projectGuid, geofenceGuid3, DateTime.UtcNow, HttpStatusCode.OK);
    ...
    [Fact]
    public async Task Try_To_Associate_Project_With_NonExistant_Customer()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      await ts.CreateProjectViaWebApiV3(projectGuid, projectId, projectName, startDate, endDate, "New Zealand Standard Time", ProjectType.ProjectMonitoring, DateTime.UtcNow, _polygon1, HttpStatusCode.OK);

      MySqlHelper.VerifyTestResultDatabaseRecordCount("Project", "ProjectUID", 1, projectGuid);
      MySqlHelper.VerifyTestResultDatabaseRecordCount("Project", "ProjectUID", 1, projectGuid);
      await ts.AssociateCustomerProjectViaWebApiV3(projectGuid, customerGuid, 1, DateTime.UtcNow, HttpStatusCode.OK);
      MySqlHelper.VerifyTestResultDatabaseRecordCount("CustomerProject", "fk_ProjectUID", 1, projectGuid);
      await ts.GetProjectsViaWebApiV3AndCompareActualWithExpected(HttpStatusCode.OK, customerGuid, new string[] { });
    }

    [Fact]
    public async Task Try_To_Associate_Project_With_Multiple_Customers()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var projectId = TestSupport.GenerateLegacyProjectId();
     ...
     $"| false      | {projectName} | New Zealand Standard Time | {ProjectType.Standard} | {startDate:O}    | {endDate:O}    | {projectGuid} | {projectId} | {customerGuid} | {projectId}      | {_polygon1} |" };
      ...
      await ts.GetProjectsViaWebApiV3AndCompareActualWithExpected(HttpStatusCode.OK, customerGuid, expectedProjects);
      await ts.AssociateCustomerProjectViaWebApiV3(projectGuid, Guid.NewGuid(), 102, DateTime.UtcNow, HttpStatusCode.BadRequest);
    }
    ...
    /// </summary>
    [Fact]
    public async Task Try_To_Associate_Geofence_With_Multiple_Projects()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var projectGuid2 = Guid.NewGuid();
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId1, projectName, startDate, endDate, 1);
      await Create_And_Subscribe_AdditionalProjects_for_existing_Customer(ts, customerGuid, projectGuid2, projectId2, projectName, startDate, endDate, 2);
      ...
      await ts.AssociateGeofenceProjectViaWebApiV3(projectGuid2, geofenceGuid, DateTime.UtcNow, HttpStatusCode.OK);
      MySqlHelper.VerifyTestResultDatabaseRecordCount("ProjectGeofence", "fk_ProjectUID", 1, projectGuid2);

      var expectedProjects = new[] {
      "| IsArchived | ProjectName    | ProjectTimezone           | ProjectType            | ProjectStartDate | ProjectEndDate | ProjectUID     | ProjectID    | CustomerUID    | LegacyCustomerId | ProjectBoundary | ",
     $"| false       | {projectName} | New Zealand Standard Time | {ProjectType.Standard} | {startDate:O}    | {endDate:O}    | {projectGuid}  | {projectId1} | {customerGuid} | 1      | {geometryWkt}   |",
    ...
    [Fact]
    public async Task Create_Project_In_The_Past()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var projectGuid2 = Guid.NewGuid();
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, 1);
      await ts.CreateProjectViaWebApiV3(projectGuid2, projectId, projectName, startDate.AddYears(-5), endDate.AddYears(-5), "New Zealand Standard Time", ProjectType.ProjectMonitoring, DateTime.UtcNow, _polygon1, HttpStatusCode.InternalServerError);
    ...
    [Fact]
    public async Task Try_To_Create_Project_Which_Ends_Before_Starts()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var projectGuid2 = Guid.NewGuid();
      ...
      await Create_Customer_Then_Project_And_Subscriptions(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, 1);
      await ts.CreateProjectViaWebApiV3(projectGuid2, projectId, projectName, endDate, startDate, "New Zealand Standard Time", ProjectType.ProjectMonitoring, DateTime.UtcNow, _polygon1, HttpStatusCode.BadRequest);
    ...
    [Fact]
    public async Task Create_Everything_Out_Of_Order()
    {
      var ts = new TestSupport { IsPublishToKafka = true };
      var projectGuid = Guid.NewGuid();
      var customerGuid = Guid.NewGuid();
      ...
      //create project
      await ts.CreateProjectViaWebApiV3(projectGuid, projectId, projectName, startDate, endDate, "New Zealand Standard Time", ProjectType.Standard, DateTime.UtcNow, geometryWKT, HttpStatusCode.OK);
      ...
      await ts.PublishEventCollection(customerEventArray); //Create customer to associate project with
    ...
    [Fact]
    public async Task CreateStandardProjectThenUpdateProjectTypeToCivilAndCheckDatabase()
    {
    ...
    private static async Task Create_Customer_Then_Project_And_Subscriptions(TestSupport ts, Guid customerGuid, Guid projectGuid, int projectId, string projectName, DateTime startDate, DateTime endDate, int numProjectsForCustomer, string timeZone = "New Zealand Standard Time")
    {
      var customerEventArray = new[] {
       "| EventType           | EventDate   | CustomerName | CustomerType | CustomerUID    |",
      $"| CreateCustomerEvent | 0d+09:00:00 | E2ECust1     | Customer     | {customerGuid} |"};
      ...
      MySqlHelper.VerifyTestResultDatabaseRecordCount("Customer", "CustomerUID", 1, customerGuid);
      MySqlHelper.VerifyTestResultDatabaseFieldsAreExpected("Customer", "CustomerUID", "name", "E2ECust1", customerGuid);
      await Create_And_Subscribe_AdditionalProjects_for_existing_Customer(ts, customerGuid, projectGuid, projectId, projectName, startDate, endDate, numProjectsForCustomer, ProjectType.Standard, timeZone);
    }

    private static async Task Create_And_Subscribe_AdditionalProjects_for_existing_Customer(TestSupport ts, Guid customerGuid, Guid projectGuid, int projectId, string projectName, DateTime startDate, DateTime endDate, int numProjectsForCustomer, ProjectType projectType = ProjectType.Standard, string timeZone = "New Zealand Standard Time")
    {
      var subscriptionUid = Guid.NewGuid();
      var eventArray = new[] {
      ...
      await ts.PublishEventCollection(eventArray);
      ...
      await ts.CreateProjectViaWebApiV3(projectGuid, projectId, projectName, startDate, endDate, timeZone, projectType, DateTime.UtcNow, _polygon1, HttpStatusCode.OK);

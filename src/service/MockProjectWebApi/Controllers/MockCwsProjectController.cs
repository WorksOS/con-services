...
namespace MockProjectWebApi.Controllers
{
  public class MockCwsProjectController : BaseController
  {
    ...
    private static Dictionary<string, Dictionary<string, ProjectDetailResponseModel>> projects = new Dictionary<string, Dictionary<string, ProjectDetailResponseModel>>();
    ...
    public CreateProjectResponseModel CreateProject([Mvc.FromBody] CreateProjectRequestModel createProjectRequestModel)
    {
      ...
      var projectDetailResponseModel = new ProjectDetailResponseModel()
      {
        ...
        {
          ProjectTRN = newProjectTrn,
          // TimeZone is generated by cws, what should we put here? or just don't test it
          Boundary = createProjectRequestModel.Boundary,
          Config = new List<ProjectConfigurationModel>(),
        }
      };

      if (!projects.ContainsKey(projectDetailResponseModel.AccountTRN))
        projects.Add(projectDetailResponseModel.AccountTRN, new Dictionary<string, ProjectDetailResponseModel>());
      projects[projectDetailResponseModel.AccountTRN].Add(newProjectTrn, projectDetailResponseModel);
    ...
    {
      foreach (var projectDict in projects)
      {
    ...
    public ProjectSummaryListResponseModel GetProjectsForCustomer(string accountTrn, [Mvc.FromQuery] int from, [Mvc.FromQuery] int limit)
    {
      ...
      if (projects.ContainsKey(accountTrn))
      {
        foreach (var projectDict in projects[accountTrn])
        {
    ...
    public CreateFileResponseModel CreateFile(string projectTrn, [Mvc.FromBody] CreateFileRequestModel createFileRequest)
    {
      ...
      filespaceIdNameMap.Add(createFileResponse.FileSpaceId, $"{projectTrn}||{DateTime.UtcNow}||{createFileRequest.FileName}");

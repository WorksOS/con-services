...
namespace VSS.Productivity3D.WebApi.Compaction.Controllers
{
  ...
  public class CompactionElevationController : BaseController<CompactionElevationController>
  {
      ...
      [FromServices] IBoundingBoxService boundingBoxService)
    {
      ...
      try
      {
        var result = await boundingBoxService.GetProductionDataExtents(projectUid, projectId, excludedIds.Select(e => e.Item1).ToArray(), excludedIds.Select(e => e.Item2).ToArray(), GetUserId(), CustomHeaders);

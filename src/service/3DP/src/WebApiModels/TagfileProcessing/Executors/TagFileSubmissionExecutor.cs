...
namespace VSS.Productivity3D.WebApi.Models.TagfileProcessing.Executors
{
  ...
  /// </summary>
  public class TagFileSubmissionExecutor : RequestExecutorContainer
  {
    /// <summary>
    /// Default constructor for RequestExecutorContainer.Build.
    /// </summary>
    public TagFileSubmissionExecutor()
    {
      ProcessErrorCodes();
    }
    ...
    protected override async Task<ContractExecutionResult> ProcessAsyncEx<T>(T item)
    {
      ...
      log.LogInformation($"{nameof(TagFileSubmissionExecutor)} completed: filename {request.FileName}  result {JsonConvert.SerializeObject(returnResult)}");
      if (returnResult.Code != 0)
        log.LogDebug($"{nameof(TagFileSubmissionExecutor)}: Failed to import tagfile '{request.FileName}', {returnResult.Message}");

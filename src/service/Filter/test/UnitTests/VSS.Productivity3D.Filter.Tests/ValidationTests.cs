...
namespace VSS.Productivity3D.Filter.Tests
{
  ...
  public class ValidationTests : ExecutorBaseTests
  {
    ...
    public async Task HydrateJsonWithBoundary_NoGeofence(FilterType filterType)
    {
      ...
      var geofenceProxy = new Mock<IGeofenceProxy>();
      geofenceProxy.Setup(g => g.GetFavoriteGeofence(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, string>>())).ReturnsAsync(
        (GeofenceData)null);
    ...
    public async Task HydrateJsonWithBoundary_InvalidBoundary(FilterType filterType)
    {
      ...
      geofenceProxy.Setup(g => g.GetFavoriteGeofence(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, string>>())).ReturnsAsync(geofenceData);
    ...
    [DataRow(FilterType.Persistent, GeofenceType.FillZone)]
    [DataRow(FilterType.Persistent, GeofenceType.Landfill)]
    [DataRow(FilterType.Persistent, GeofenceType.Project)]
    [DataRow(FilterType.Persistent, GeofenceType.Borrow)]
    [DataRow(FilterType.Persistent, GeofenceType.Waste)]
    [DataRow(FilterType.Persistent, GeofenceType.AvoidanceZone)]
    [DataRow(FilterType.Persistent, GeofenceType.Stockpile)]

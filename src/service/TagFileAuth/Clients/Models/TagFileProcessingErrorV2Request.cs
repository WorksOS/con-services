...
namespace VSS.Productivity3D.TagFileAuth.Models
{
  ...
  public class TagFileProcessingErrorV2Request
  {
    ...
    public string DeviceTypeString()
    {
      ...
      var isDeviceTypeValid = (( TagFileDeviceTypeEnum) deviceType).ToString() != deviceType.ToString();

      deviceTypeString = isDeviceTypeValid ? (( TagFileDeviceTypeEnum)deviceType).ToString() : string.Format($"Invalid {deviceType}");

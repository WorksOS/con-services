...
namespace VSS.TRex.GridFabric.Requests
{
  ...
  public abstract class GenericPSNodeBroadcastBinarizableRequest<TArgument, TComputeFunc, TResponse> : CacheComputePoolBinarizableRequest<TArgument, TResponse>
    where TComputeFunc : IComputeFunc<TArgument, TResponse>, new()
    where TResponse : class, IAggregateWith<TResponse>
  {
    /// <summary>
    /// Executes a request through its generic types
    /// </summary>

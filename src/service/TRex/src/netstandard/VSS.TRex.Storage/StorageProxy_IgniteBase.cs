...
namespace VSS.TRex.Storage
{
  public abstract class StorageProxy_IgniteBase
  {
    ...
    /// </summary>
    protected async Task<bool> PerformNonSpatialImmutabilityConversion(IStorageProxyCache<INonSpatialAffinityKey, ISerialisedByteArrayWrapper> mutableCache,
      ...
      FileSystemStreamType streamType)
    {
      ...
      await using var ms = new MemoryStream((await mutableCache.GetAsync(cacheKey)).Bytes);
      await using var mutableStream = MemoryStreamCompression.Decompress(ms);
      return await PerformNonSpatialImmutabilityConversion(mutableStream, immutableCache, cacheKey, streamType, null);
    }

    /// <summary>
    /// Supports taking a mutable version of a piece of data and transforming it into the immutable form if not present in the immutable cache
    /// </summary>
    protected async Task<bool> PerformNonSpatialImmutabilityConversion(MemoryStream mutableStream,
      IStorageProxyCache<INonSpatialAffinityKey, ISerialisedByteArrayWrapper> immutableCache,
      INonSpatialAffinityKey cacheKey,
      FileSystemStreamType streamType,
      object source)
    {
      ...
      try
      {
        ...
        {
          await using var compressedStream = MemoryStreamCompression.Compress(immutableStream);
          ...
          await immutableCache.PutAsync(cacheKey, new SerialisedByteArrayWrapper(compressedStream.ToArray()));
      ...
      finally
      {
        ...
        {
          await immutableStream.DisposeAsync();
        }
      }
    }
    ...
    /// Supports taking a mutable version of a piece of data and transforming it into the immutable form if not present in the immutable cache
    /// </summary>
    protected async Task PerformSpatialImmutabilityConversion(IStorageProxyCache<ISubGridSpatialAffinityKey, ISerialisedByteArrayWrapper> mutableCache,
      ...
      FileSystemStreamType streamType)
    {
      ...
      await using MemoryStream ms = new MemoryStream((await mutableCache.GetAsync(cacheKey)).Bytes), mutableStream = MemoryStreamCompression.Decompress(ms);

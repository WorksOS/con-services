...
namespace VSS.TRex.Gateway.Common.Proxy
{
  ...
  public class TRexCompactionDataProxy : BaseProxy, ITRexCompactionDataProxy
  {
    ...
    /// </summary>
    public Task<TResponse> SendDataDeleteRequest<TResponse, TRequest>(TRequest dataRequest, string route, IDictionary<string, string> customHeaders = null, bool mutableGateway = false) where TResponse : ContractExecutionResult
    {
      var request = JsonConvert.SerializeObject(dataRequest);

      log.LogDebug($"{nameof(TRequest)}: Sending the request: {request}");

      return SendRequestDelete<TResponse>(request, customHeaders, route, mutableGateway ? TREX_GATEWAY_MUTABLE_BASE_URL : TREX_GATEWAY_IMMUTABLE_BASE_URL);
    }
    ...
    /// <summary>
    /// Executes a DELETE request against the TRex Gateway service.
    /// </summary>
    private async Task<T> SendRequestDelete<T>(string payload, IDictionary<string, string> customHeaders, string route, string baseUrl = TREX_GATEWAY_IMMUTABLE_BASE_URL) where T : ContractExecutionResult
    {
      var response = await SendRequest<T>(baseUrl, payload, customHeaders, route, HttpMethod.Delete, string.Empty);

      log.LogDebug($"{nameof(SendRequestDelete)}: response: {(response == null ? null : JsonConvert.SerializeObject(response))}");

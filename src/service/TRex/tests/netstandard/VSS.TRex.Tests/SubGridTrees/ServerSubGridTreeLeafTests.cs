...
namespace VSS.TRex.Tests.SubGridTrees
{
  public class ServerSubGridTreeLeafTests : IClassFixture<DILoggingFixture>
  {
    ...
    [Fact()]
    public void Test_ServerSubGridTreeLeaf_LowestPassOnly()
    {
      var leaf = CreateADefaultEmptyLeaf();
      var BASE_TIME = DateTime.UtcNow;
      int TIME_INCREMENT_SECONDS = 10; // seconds
      float BASE_HEIGHT = 100.0f;

      // Add three passes, each separated by 10 seconds and ascending by 1m each pass, using lowestPassOnly set to true
      for (int i = 0; i < 3; i++)
      {
      ...
      Assert.True(leaf.Directory.SegmentDirectory.First().Segment.PassesData.PassCount(0, 0) == 1, "cell pass count is not 1");
      Assert.True(leaf.Directory.SegmentDirectory.First().Segment.PassesData.Pass(0, 0,0).Height == BASE_HEIGHT, "cell pass is not lowest");

      var leaf2 = CreateADefaultEmptyLeaf();

      // Repeat test again but this time lowestPassOnly = false
      for (int i = 0; i < 3; i++)
      {
        leaf2.AddPass(0, 0, new CellPass
        {
          InternalSiteModelMachineIndex = 0,
          Time = BASE_TIME.AddSeconds(i * TIME_INCREMENT_SECONDS),
          Height = BASE_HEIGHT + i,
      ...
      Assert.True(leaf2.Directory.SegmentDirectory.First().Segment.PassesData.PassCount(0, 0) == 3, "cell pass count is not 3");
    }

...
namespace TAGFiles.Tests
{
  public class CSDSwatherTests : IClassFixture<DITagFileFixture>
  {
    ...
    public void Test_CSDSwather_PerformSwathing()
    {
      ...
      var fence = new Fence();
      fence.SetRectangleFence(2.5, -2.5, 7.5, 2.5);

      CSDSwather swather = new CSDSwather(processor, MachineTargetValueChangesAggregator, siteModel, grid, fence);
      // Note! CSD moves side to side. blade left and right positions respresent the diameter of the sphere
      // The blade positions will be rotated 90 degrees to calcelate the swath path so in this example movement is along the x axis
      ...
      Assert.Equal(2, grid.Root.CountChildren());
      ...
      Assert.Equal(374, nonNullCellCount);

...
namespace VSS.TRex.Designs.TTM.Optimised
{
    public class TTMTriangle : Triangle
    {
        ...
        public void Write(BinaryWriter writer, TTMHeader header)
        {
            for (int i = 0; i < 3; i++)
            {
                Utilities.WriteInteger(writer, GetVertex(i).Tag, header.VertexNumberSize);
            }

            for (int i = 0; i < 3; i++)
            {
                int NeighbourIndex = GetNeighbour(i) == null ? Consts.NoNeighbour : GetNeighbour(i).Tag;
        ...
        TriVertices vertices, TTMTriangles triangles, int TriNumber )
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                SetVertex(i, (VertIndex < 1 || VertIndex > vertices.Count) ? null : vertices[VertIndex - 1]);
            ...
            for (int i = 0; i < 3; i++)
            {
              ...
              SetNeighbour(i, (NeighbourIndex < 1 || NeighbourIndex > triangles.Count) ? null : triangles[NeighbourIndex - 1]);

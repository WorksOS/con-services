    ...
    public class TriangleMesh
    {
        ...
        public void BuildTriangleLinks()
        {
            ...
            for (int i = 0; i < Triangles.Count; i++)
            {
                ...
                {
                    Trianglelists[Triangles[i].GetVertex(Side).Tag].Add(Triangles[i]);
                }
            }
            ...
            for (int i = 0; i < Triangles.Count; i++)
            {
              for (int Side = 0; Side < 3; Side++)
              {
                if (Triangles[i].GetNeighbour(Side) != null)
                  continue;

                TriVertex FromVertex = Triangles[i].GetVertex(Side);
                TriVertex ToVertex = Triangles[i].GetVertex(XYZ.NextSide(Side));

                Triangle Nbr = FindNeighbour(Trianglelists, Triangles[i], FromVertex, ToVertex);
                Triangles[i].SetNeighbour(Side, Nbr);
                if (Nbr != null)
                {
                  int NbrSide = Nbr.GetSideIndex(FromVertex, ToVertex);
                  if (Nbr.GetNeighbour(NbrSide) == null)
                  {
                    Nbr.SetNeighbour(NbrSide, Triangles[i]);
                  }
                  else
                  {
                    Triangles[i].SetNeighbour(Side, null);
                  }
                }
              }
            }

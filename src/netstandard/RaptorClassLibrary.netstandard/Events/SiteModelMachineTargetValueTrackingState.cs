...
namespace VSS.TRex.Events
{
  ...
  public struct SiteModelMachineTargetValueTrackingState<T>
  {
    ...
    public bool IsNextEventSuitable(int stamp, DateTime _time, ProductionEvents<T> eventList)
    {
      ...
      if (NewIndex < EventListCount)
      {
        ...
        {
          NewNextEvent = eventList.Events[NewNextEventIndex].State;
          NewEndDate = eventList.Events[NewNextEventIndex].Date;
        }
        else
        {
    ...
    public void RecordEventState(int stamp, ProductionEvents<T> eventList)
    {
      ...
      if (Index < 0)
      {
        ...
        if (EventListCount > 0)
        {
          ThisEvent = eventList.Events[0].State;
          EndDate = eventList.Events[0].Date;
        }
        else
        {
          ThisEvent = default;
      ...
      {
        ThisEvent = eventList.Events[Index].State;
        StartDate = eventList.Events[Index].Date;

        if (Index < EventListCount - 1)
        {
          NextEvent = eventList.Events[Index + 1].State;
          EndDate = eventList.Events[Index + 1].Date;
        }
        else
        {
          NextEvent = default;
    ...
    /// <returns></returns>
    public T DetermineTrackingStateValue(int stamp, DateTime _Time, ProductionEvents<T> events)
    {
      T result = default;

...
/*
 Web Mercator class. Cesium uses Web Mercator projection imagery by default. Our ALK maps are also in Web Mercator 
 Web Mercator is a coordinate system in metres. Therefore we need to convert our WGS84 (EPSG:4326) lat long coordinates into a Web Mercator projection (EPSG:3857)
 For a good visual explanation see https://www.maptiler.com/google-maps-coordinates-tile-bounds-projection/
 */
...
namespace VSS.Map3D.Common
{
  ...
  public class WebMercator
  {
    ...
    public static PointD PixelsToMeters(PointF p, int zoom)
    {
    ...
    public static PointD MetersToPixels(PointD m, int zoom)
    {
      var res = Resolution(zoom);
      var pix = new PointD();
    ...
    public static Tile PixelsToTile(PointD p)
    {
      var t = new Tile();
    ...
    public static PointF PixelsToRaster(PointF p, int zoom)
    {
      var mapSize = TileSize << zoom;
      return new PointF(p.X, mapSize - p.Y);
    }

    //Returns tile for given mercator coordinates
    public static Tile MetersToTile(PointD m, int zoom)
    {
    ...
    public static RectangleF TileBounds(Tile t, int zoom)
    {
      ...
      return new RectangleF(minPf, szF);
    }

    //Returns bounds of the given tile in latutude/longitude using WGS84 datum
    public static RectangleF TileLatLonBounds(Tile t, int zoom)
    {
      var bound = TileBounds(t, zoom);
      var min = MetersToLatLon(new PointD(bound.Left, bound.Top));
      var max = MetersToLatLon(new PointD(bound.Right, bound.Bottom));
    ...
    public static string LatLonToQuadTree(PointD latLon, int zoom)
    {
      PointD m = LatLonToMeters(latLon.Y, latLon.X);
    ...
    public static RectangleF QuadTreeToLatLon(string quadtree)
    {
    ...
    public static List<string> GetQuadTreeList(int zoom, PointD latLonMin, PointD latLonMax)
    {
      if (latLonMax.Y < latLonMin.Y || latLonMax.X < latLonMin.X)
        return null;

      PointD mMin = LatLonToMeters(latLonMin.Y, latLonMin.X);
      Tile tmin = MetersToTile(mMin, zoom);
      PointD mMax = LatLonToMeters(latLonMax.Y, latLonMax.X);
    ...
    public class MapPoint
    {
      ...
      public MapPoint(double x, double y)//, SpatialReference sp)
      {
        this.x = x;
        this.y = y;
      }

...
namespace VSS.Productivity3D.Common.Filters
{
  public class ExceptionsTrap
  {
    ...
    public async Task Invoke(HttpContext context)
    {
      ...
      {
        _log.LogWarning($"Service exception: {ex.GetContent} statusCode: {ex.Code}");
      ...
      catch (Exception ex)
      {
        ...
        {
          _log.LogCritical("EXCEPTION: {0}, {1}, {2}", ex.Message, ex.Source, ex.StackTrace);

          if (ex is AggregateException exception)
          {
            _log.LogCritical("EXCEPTION AGGREGATED: {0}, {1}, {2}",

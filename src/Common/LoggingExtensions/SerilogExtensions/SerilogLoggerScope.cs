...
{
  /// <summary>
  /// This is from serilog.extensions.logging package;
  /// https://github.com/serilog/serilog-extensions-logging/blob/dev/src/Serilog.Extensions.Logging/Extensions/Logging/SerilogLoggerScope.cs
  /// </summary>
  internal class SerilogLoggerScope : IDisposable
  {
    ...
    public SerilogLoggerScope(SerilogProvider provider, object state, IDisposable chainedDisposable = null)
    {
      _provider = provider;
      _state = state;
    ...
    public void EnrichAndCreateScopeItem(LogEvent logEvent, ILogEventPropertyFactory propertyFactory, out LogEventPropertyValue scopeItem)
    {
      ...
      if (stateProperties != null)
      {
        ...
        {
          if (stateProperty.Key == SerilogProvider.OriginalFormatPropertyName && stateProperty.Value is string)
          {

...
namespace VSS.MasterData.Proxies
{
  ...
  public class TPaasProxy : BaseProxy, ITPaasProxy
  {
    ...
    public async Task<TPaasOauthResult> GetApplicationBearerToken(string grantType, Dictionary<string, string> customHeaders)
    {
      ...
      try
      {
        tPaasOauthResult.tPaasOauthRawResult = await SendRequest<TPaasOauthRawResult>("TPAAS_OAUTH_URL", payLoadToSend, customHeaders, null, HttpMethod.Post, string.Empty);
      }
      catch (Exception e)
      {

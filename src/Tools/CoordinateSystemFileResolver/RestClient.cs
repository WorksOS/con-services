...
namespace CoordinateSystemFileResolver
{
  public class RestClient : IRestClient
  {
    ...
    /// </summary>
    public TResponse SendHttpClientRequest<TResponse>(
      string uri,
      HttpMethod method,
      string acceptHeader,
      string contentType,
    ...
    {
      Log.LogInformation($"URI: {method} {uri}");
      ...
      try
      {
        ...
        switch (response.StatusCode)
        {
          case HttpStatusCode.OK:
            {
              Log.LogInformation($"Status [{response.StatusCode}] Body: '{responseBody}'");
              break;
            }
          case HttpStatusCode.InternalServerError:
          case HttpStatusCode.NotFound:
            {
              Log.LogError($"Status [{response.StatusCode}] Body: '{responseBody}'");
              Debugger.Break();
          ...
          default:
            {
              Log.LogDebug($"Status [{response.StatusCode}] URI: '{request.RequestUri.AbsoluteUri}', Body: '{responseBody}'");
        ...
        switch (response.Content.Headers.ContentType.MediaType)
        {
          ...
          case MediaType.APPLICATION_OCTET_STREAM:
            {
              return response.Content.ReadAsStringAsync().Result as TResponse;
            }
          default:
            {
      ...
      {
        Log.LogError($"{method} URI: '{request.RequestUri.AbsoluteUri}', Exception: {exception.GetBaseException()}");
      }

...
namespace VSP.MasterData.Project.WebAPI.Controllers.V4
{
  public class ProjectV4Controller : ProjectV3Controller
  {
    ...
    public ProjectV4Controller(IKafka producer, IRepository<IProjectEvent> projectRepo,
        IRepository<ISubscriptionEvent> subscriptionsRepo, IConfigurationStore store, ILoggerFactory logger) : base (producer,projectRepo,subscriptionsRepo,store,logger)
    {
    }
    ...
    {
      var overlaps = await _projectService.DoesPolygonOverlap(project.CustomerUID.ToString(), databaseProjectBoundary, project.ProjectStartDate, project.ProjectEndDate).ConfigureAwait(false);
      if (overlaps)
      {

...
namespace VSP.MasterData.Project.WebAPI.Controllers.V4
{
  public class FileImportV4Controller : ProjectBaseController
  {
    ...
    [HttpPost]
    public async Task<ImportedFileDescriptorSingleResult> CreateImportedFileV4([FromBody] CreateImportedFileRequest importedFileRequest)
    {
      ...
      log.LogInformation($"CreateImportedFileV4. importedFileRequest: {JsonConvert.SerializeObject(importedFileRequest)}");

      // todo

      var response = new ImportedFileDescriptorSingleResult(AutoMapperUtility.Automapper.Map<ImportedFileDescriptor>
      ...
      log.LogInformation($"CreateImportedFileV4. completed succesfully. Response: {JsonConvert.SerializeObject(response)}");
      return response;
    }
    ...
    [Route("api/v4/importedfile")]
    [HttpPut]
    public async Task<ImportedFileDescriptorSingleResult> UpdateImportedFileV4([FromBody] UpdateImportedFileRequest importedFileRequest)
    {
      if (importedFileRequest == null)
      {
        throw new ServiceException(HttpStatusCode.InternalServerError,
      ...
      log.LogInformation($"UpdateImportedFileV4. importedFileRequest: {JsonConvert.SerializeObject(importedFileRequest)}");

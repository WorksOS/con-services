...
namespace VSS.Customer.Data
{
  public class MySqlUserCustomerRepository : IUserCustomerService
  {
    ...
    /// <returns></returns>
    private int UpsertUserCustomerDetail(UserCustomer userCustomer, string eventType)
    {
      int upsertedCount = 0;
      using (var connection = new MySqlConnection(_connectionString))
      {
        ...
        connection.Open();
        var existing = connection.Query<UserCustomer>
          (@"SELECT 
    ...
    public int AssociateCustomerUser(MySqlConnection connection, UserCustomer userCustomer, UserCustomer existing)
    {
      if (existing == null)
      {
    ...
    public int DissociateCustomerUser(MySqlConnection connection, UserCustomer userCustomer, UserCustomer existing)
    {
      if (existing != null)
      {
    ...
    public UserCustomer GetUserCustomer(int UserCustomerID)
    {
      using (var connection = new MySqlConnection(_connectionString))
      {
        connection.Open();

        var userCustomer = connection.Query<UserCustomer>
          (@"SELECT *
              FROM UserCustomer
    ...
    public IEnumerable<UserCustomer> GetUserCustomers()
    {
      ...
      using (var connection = new MySqlConnection(_connectionString))
      {
        ...
        userCustomers = connection.Query<UserCustomer>
          (@"SELECT 
